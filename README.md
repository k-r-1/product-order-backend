...
- [x] **재고 차감 로직 및 원자성 확보**
  ...
  - 조건문을 통해 재고가 부족해지면 0 미만으로 내려가지 않도록 방어했습니다.
  - 추가로 2명 이상이 동시에 주문할 때 재고가 꼬이는 동시성 문제를 해결하기 위해, DB 자체의 **비관적 락(Pessimistic Lock)** 방식을 활용하여 완벽한 원자적(Atomic) 차감 처리를 보장했습니다.

## 5. 추가로 공부하고 적용해 본 내용 (Troubleshooting)
